...
void searchName(const vector<librarianInfo>& data, const string& name){
    bool found = false;
    for(static auto& employee: data){
        if(employee.fullName == name){
            found = true;
        }
    }
    if(found == true){
            cout<<"That name exist.";
        }
    if(!found){
...
void removeEmployeeData(vector<librarianInfo>& data, int index, const string& fileName){
    if(index >= 0 && index <= data.size()){
        data.erase(data.begin()+index, data.end());
        // Update the .txt file with the modified data
        ofstream file(fileName);
        if (file.is_open()) {
            int i = 1;
            for (const auto& employee : data) {
                file << "\nName: " << employee.fullName << endl;
                file << "Date of birth: " << employee.dateOfBirth.day << "/" << employee.dateOfBirth.month << "/" << employee.dateOfBirth.year << endl;
                file << "Email: " << employee.email << endl;
                file << "Password: " << employee.password << endl;
                file << "Education: " << employee.educationLevel << endl;
                ...
                file << "Phone number: " << employee.phoneNo << endl;
                file << "Employee ID: " << employee.ID << endl;
                file << "Employee role: " << employee.role << endl;

                i++;
            }
            file.close();
            cout << "\nSuccessfully removed employee data and updated the file!\n"<<i;
    }
    }
    else{
        cout<<"\nWrong index!\n";
