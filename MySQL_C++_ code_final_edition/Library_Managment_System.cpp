...
void registeradmin(MYSQL* conn){
  ...
  //  string firstName, lastName, gender, username, email, password;
   ...
   cout << "Enter Email: ";
    ...
    // Register the admin
    registerAdmin(conn, user.first_name,  user.last_name, user.gender,  user.username, user.email,  user.password);
...
void registerstudent(MYSQL* conn){
 ...
 //   string firstName, lastName, birthdate, gender, grade, username, email, password;
   ...
   gotoxy(55,28);
    ...
    if (registerStudent(conn, user.first_name, user.last_name, user.birthdate,user.gender, user.grade, user.username, user.email, user.password)) {
         gotoxy(50,36);
    SetConsoleTextAttribute(hConsole, 10);
...
void accountSetting_admin(MYSQL* conn) {
    ...
    string query = "SELECT * FROM admin WHERE email = '" + user_email + "' AND password = '" + user_password + "'";
